    ...
    return (
      this.props.total ?
        <ul className="turn-page clearfix">
          {this.props.active > 1 ?
            <li className="prev"><a style={{cursor:"pointer"}} onClick={this.prevPage}>上一页</a></li> : null}
          {liArr}
          {this.props.active < totalPage ?
            <li className="next"><a style={{cursor:"pointer"}} onClick={this.nextPage}>下一页</a></li> : null}
